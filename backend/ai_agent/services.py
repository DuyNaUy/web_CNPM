...
class AIAgentService:
    ...
    def __init__(self):
        ...
        self.gemini_model = getattr(settings, 'GEMINI_MODEL', 'gemini-3-flash-preview') or os.getenv('GEMINI_MODEL', 'gemini-3-flash-preview')
    ...
    def _build_system_prompt(self) -> str:
        """
        Xây dựng system prompt mặc định (gần với trạng thái ban đầu khi mới cài).
        """
        products_info = self._get_product_catalog_summary()
        base_prompt = """Bạn là một tư vấn viên bán hàng của TeddyShop (chuyên gấu bông và quà tặng).

Nhiệm vụ của bạn:
1. Tư vấn sản phẩm phù hợp theo nhu cầu khách hàng dựa trên dữ liệu catalog được cung cấp.
2. Trả lời rõ ràng, thân thiện, bằng tiếng Việt.
3. Khi thiếu thông tin, nói rõ chưa có dữ liệu và hỏi thêm để tư vấn chính xác hơn.
4. Không bịa thông tin ngoài dữ liệu hiện có.

DỮ LIỆU CATALOG THAM KHẢO:
"""
        ...
        base_prompt += "\n\nLưu ý: Không tự bịa thông tin sản phẩm ngoài dữ liệu hiện có."
    ...
    def chat(self, conversation: ConversationSession, user_message: str, user_products: Optional[List[Dict]] = None) -> Dict:
        """
        ...
        """
        # Lưu tin nhắn từ user (hỗ trợ kèm thẻ sản phẩm phía client)
        conversation.add_message('user', user_message, products=user_products or [])
...
Trợ lý:"""
    ...
    def _align_response_with_products(self, original_response: str, products: List[Dict], user_message: str = None) -> str:
        """
        Trả về phản hồi gốc của AI để giữ cách trả lời tự nhiên mặc định.
        Chỉ fallback sang câu mặc định khi phản hồi rỗng.
        """
        ...
        if products:
            return "Mình đã tìm được một số sản phẩm phù hợp, bạn xem giúp mình các sản phẩm bên dưới nhé."

        return "Mình đã nhận yêu cầu của bạn. Bạn mô tả thêm nhu cầu để mình tư vấn chính xác hơn nhé."
    ...
    def _clean_response_text(self, text: str, product_names: List[str]) -> str:
        """
        Giữ nguyên text AI gần mặc định, chỉ chuẩn hóa khoảng trắng cơ bản.
        """
        if text is None:
            return ""
